...
export function getLeaderboardByScoresRouteHandler(req: IAuthenticatedRequest, res: exp.Response): void {
  const iocContainer = req.requestIocContainer;
  const log = iocContainer.get<ILoggerFactory>(Symbol.for('ILoggerFactory')).getLogger('action.leaderboard.getLeaderboardByScoresRouteHandler');

  let leaderboardService = iocContainer.get<ILeaderboardService>(Symbol.for('ILeaderboardService'));
  leaderboardService.getLeaderboardByScores()
    .then((results: any) => {
