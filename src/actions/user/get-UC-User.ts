...
import { IUnifiedSchemaService } from '../../services/unified-schema';
import * as validate from '../../services/validation';
import * as uidUtil from 'library-uid';

export function getUCUserRouteHandler(req: IAuthenticatedRequest, res: exp.Response): void {
  const iocContainer = req.requestIocContainer;
  const log = iocContainer.get<ILoggerFactory>(Symbol.for('ILoggerFactory')).getLogger('action.user.getUCUserRouteHandler');
  ...
  let unifiedSchemaService = iocContainer.get<IUnifiedSchemaService>(Symbol.for('IUnifiedSchemaService'));
  unifiedSchemaService.getStudentDetails(userUid)
    .then((results: IUser) => {
